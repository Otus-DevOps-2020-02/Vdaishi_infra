# Vdaishi_infra [![Build Status](https://travis-ci.com/Otus-DevOps-2020-02/vdaishi_infra.svg?branch=master)](https://travis-ci.com/Otus-DevOps-2020-02/vdaishi_infra)
...
# Добавление pre-commit в репу
Требуется создать файл `pre-commit-config.yaml` со следующим содержимым в директории `.github`, после чего требуется установить в данную директорию `pre-commit`:
1. Создаем директорию `.github`
...
2. создаем в директории `.github` файл `pre-commit-config.yaml`
...
Чтобы можно было работать любому приложению с авторизацией в GCP требуется для каждой машины, на которой вы будете производить конфигурацию, произвести аутентификацию
...
Дабы упростить разработку инфаструктурного кода, лучшим решением разделить плейбуки на роли, упрощая версионирование и использование кода для разных окружений.
Привер расположения данных на примере роли db:
...
Для вызова роли необходимо в плейбуке это явно указать
...
Разные окружения могут иметь небольшие отличия (разные переменные) для которых необходимо создать собственные настройки. Для этого требуется создать отдельную директорию `environments` и хранить там необходимые данные (переменные, инвентори файлы, требования к модулям, и тп..)
Пример древа окружений для прода и тест сред:

 ```zsh
environments
├── prod # <-- Директория для прод окружения
│   ├── credentials.yml
│   ├── gcpkey.json
│   ├── group_vars
│   │   ├── all
...
│   ├── inventory.gcp.yml
│   └── requirements.yml
└── stage # <-- Директория для тестовой среды
    ├── credentials.yml
    ├── gcpkey.json
    ├── group_vars
    │   ├── all
 ...
 Чтобы понимать, какая конфигурация сейчас используется и для какой среды, используется переменная `env`, которая к примеру может быть вызвана во время прохождения плейбука:
...
Указывая в файле `requirements.yml`, какие роли, созданные другими пользователями, необходимы для работы окружения.

В данной работе рассматривается использование роли `jdauphant.nginx` для управления веб сервером `nginx` и проксированием нашего приложения на порт `80`

### Использование Ansible Vault

мы можем использовать плейбук для передачи чувствительных данных, таких как связки пользователь и пароль. Но так как хранить такие данные в открытом виде опасно, их лучше всего шифровать. Для этого можно использовать `Ansible Vault`, с помощью которого `Ansible` сможет дешифровывать данные.

Для этого необходимо создать файл `vault.key` и хранить его в мало доступном месте а так же указать в конфиге путь до этого ключа:
...
Шифровать файл можно с помощью команды :

```zsh
ansible-vault encrypt <file>
```

Для редактирования переменных в файле нужно использовать команду :

```
ansible-vault edit <file>
```

Для расшифровки файла необходимо использовать команду:

```
 ansible-vault decrypt <file>
```

### Работа с динамическим инвентори в окружениях

при использовании динамического окружения можно автоматически получать все необходимые переменные, такие как внутренние и внешние адреса инстансов. Благодаря этому не надо постоянно менять данные в инвентори файлах и в переменных.

Объявление переменных происходит так же как при создании переменных в Terraform:
...
Переменная для app плейбука:

```
db_host: "{{ hostvars[groups['db'][0]]['ansible_private_host'] }}"
```

# Разработка итестирование Ansible ролей и плейбуков

- Локальная разработка при помощи Vagrant, доработка ролей для провижининга в Vagrant
- Тестирование ролей при помощи Molecule иTestinfra
- Переключение сбора образов пакером наиспользование ролей

### Описание локальной инфраструктуры

За характеристики ВМ , которые мы собираемся создать отвечает файл `Vagrantfile`

В случае, если у нас нет указанного бокса (образа) на локальной машине, то Vagrant скачает его самостоятельно из `Vagrant Cloud`

C помощью команды `vagrant box list`  мы можем посмотреть список образов, которые есть на нашей локальной машине.
Чтобы создать все машины, используется команда `vagrant up`
С помощью команды `vagrant status` мы можем посмотреть машины, запущенные в VirtualBox.

Чтобы уничтожить созданные ВМ используется команда `vagrant destroy`. Ключ `-f` уничтожает машины без предупреждения.

### Работа с провижинерами

В Vagrant можно указать провижинеры, которые будут исполняться при настройке ВМ
...
чтобы запустить провижинер без пересоздания ВМ, необходимо использовать команду `vagrant provision your_server`

### задание со * (1)

Для того, чтобы Vagrant передал необходимые переменные роли `jdauphant.nginx` необходимо указать следующие переменные (разница между прежними файлами из окружений Ansible чисто в пунктуации)

PS. У одной ВМ не может быть несколько `extra_vars`, работать будет только последний из них
...
Данные файлы можно прописать в файл requirements.txt и установить их.

Будьте внимательны, в Molecule выше 3.0 отсутствует ключ ``--scenario-name``, который более не требуется вводить при инициализации заготовки тестов

```
molecule init scenario default -r db -d vagrant
```

С помощью `molecule create` создается ВМ , на которой проводится тестирование

Проверка инстансов на которые вы можете подключиться проверяются `molecule list`

С помощью `molecule login -h instance` можно подключиться к созданной ВМ.

Применение конфигурации происходит с помощью команды `molecule converge`

Прогон тестов с помощью `molecule verify`

Уничтожение созданных molecule VM - `molecule destroy`
